...
void i8048_reset(struct i8048_state_S *state) {
  ...
  // Expander ports and BUS latch reset to 0
  state->p4 = 0;
  state->p5 = 0;
  state->p6 = 0;
  state->p7 = 0;
  state->bus_latch = 0xFF;  // BUS port default high
...
static void handleInterrupts(struct i8048_state_S *state) {
  if (!state->inInterrupt) {
    ...
    if (!state->notINT && state->externalInterruptsEnabled) {
      // handle external interrupt (edge-triggered: auto-clear after acknowledge)
      push(state);
      state->PC = 3;
      state->inInterrupt = true;
      state->notINT = true;  // edge-triggered: one-shot, prevents re-entry loop
...
void i8048_step(struct i8048_state_S *state) {
  ...
  case 0x86: // JNI addr — Jump if INT pin is LOW (active)
    // MAME: execute_jcc(m_irq_state != 0) — jumps when INT IS active.
    // notINT=false means INT active, so jump when !notINT.
