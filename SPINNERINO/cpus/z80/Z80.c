...
/* Quando !=0, il bit P/V delle operazioni aritmetiche e' la PARITA' del
   risultato (Intel 8080) invece dell'overflow (Z80). Vedi Z80.h. */
byte Z80_i8080 = 0;

#define M_PV8080(res)  (PZSTable[res]&P_FLAG)

#define M_ADD(Rg)      \
  J.W=R->AF.B.h+Rg;    \
  R->AF.B.l=           \
    (Z80_i8080? M_PV8080(J.B.l) : (~(R->AF.B.h^Rg)&(Rg^J.B.l)&0x80? V_FLAG:0))| \
    J.B.h|ZSTable[J.B.l]|                        \
...
#define M_SUB(Rg)      \
  J.W=R->AF.B.h-Rg;    \
  R->AF.B.l=           \
    (Z80_i8080? M_PV8080(J.B.l) : ((R->AF.B.h^Rg)&(R->AF.B.h^J.B.l)&0x80? V_FLAG:0))| \
...
#define M_INC(Rg)       \
  ...
  R->AF.B.l=            \
    ...
    (Z80_i8080? M_PV8080(Rg) : (Rg==0x80? V_FLAG:0))|(Rg&0x0F? 0:H_FLAG)

#define M_DEC(Rg)       \
  Rg--;                 \
  R->AF.B.l=            \
    N_FLAG|(R->AF.B.l&C_FLAG)|ZSTable[Rg]| \
    (Z80_i8080? M_PV8080(Rg) : (Rg==0x7F? V_FLAG:0))|((Rg&0x0F)==0x0F? H_FLAG:0)
...
int ExecZ80(register Z80 *R,register int RunCycles)
{
  ...
  for(R->ICount=RunCycles;;)
  {
    while(R->ICount>0)
    {
      ...
      /* Read opcode and count cycles (M1 cycle - use opcode space) */
      I=OpZ80_INL(R->PC.W++);
      /* Count cycles */
      R->ICount-=Cycles[I];
